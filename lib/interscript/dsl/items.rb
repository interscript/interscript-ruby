module Interscript::DSL::Items
  ...
  def any(*chars)
    puts "any(#{chars.inspect}) from #{inspect}" if $DEBUG
    Interscript::Node::Item::Any.new(*chars)
  end

  # a?
  def maybe(*chars)
    puts "maybe(#{chars.inspect}) from #{inspect}" if $DEBUG
    Interscript::Node::Item::Maybe.new(*chars)
  end

  def maybe_some(*chars)
    puts "maybe_some(#{chars.inspect}) from #{inspect}" if $DEBUG
    Interscript::Node::Item::MaybeSome.new(*chars)
  end

  def some(*chars)
    puts "some(#{chars.inspect}) from #{inspect}" if $DEBUG
    Interscript::Node::Item::Some.new(*chars)
  end

  # (...)
  def capture(expr)
    puts "capture(#{expr.inspect}) from #{inspect}" if $DEBUG
    Interscript::Node::Item::CaptureGroup.new(expr)
  end

  # \1
  def ref(int)
    puts "ref(#{int.inspect}) from #{inspect}" if $DEBUG
    Interscript::Node::Item::CaptureRef.new(int)
  end

  # Implementation of `map.x`
  def map
    Interscript::DSL::Items::Maps
  end
